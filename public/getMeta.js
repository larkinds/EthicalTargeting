...
chrome.storage.sync.get(function(userInfo) {
    ...
    for (var i=0; i<metas.length; i++) {
        ...
        for(let index in sentence){     
            ...
            if(word.length < 4) continue;                                // If the word has only 1 character, its probably irrelevant, skip
            if (keywordsDict.hasOwnProperty(word)) {
                keywordsDict[word]["weight"]++; // If the word already exists in the dict, increment its value
            }
            else {
                keywordsDict[word] = {"name": word,"weight":1};         // else, adds the word to the dict with an initial value of 1.
            }                                  
        }
    }
